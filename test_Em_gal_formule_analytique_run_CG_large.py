...
#for i_E, E in enumerate(energy_bins[0:1]):
    ...
    lon=8 / pix_deg#Pour aller a plus ou moins 2deg as did Anne
    ...
    """
    name_interest="box(197.71,202.81,350,250)"
    notice2d(name_interest)
    name="box(185.20985,202.81,125,80)"
    ignore2d(name)
    ...
    width=100
    height=80
    name2 = "box(" + str(source_J1745_303_xpix+20) + "," + str(source_J1745_303_ypix-20) + "," + str(width) + "," + str(height) +")"
    ignore2d(name2)
    name3 = "circle(287,208,20)"
    ignore2d(name3)
    name4 = "circle(77,204,20)"
    ignore2d(name4)
    name5 = "circle(119,203,10)"
    ignore2d(name5)
    name6 = "circle(252,200,15)"
    ignore2d(name6)
    #name_region2 = "circle(11,292,20)"
    ...
    if param_fit["Em_gal"]:
        ...
        EmGal=UniformGaussianPlane(wcs=on.wcs)
    ...
    pt.savefig("resid_Em_gal_formule_analytique"+config_name+".png")
    print("Counts data: "+str(map_data.data.sum()))
    print("Counts model: "+str(model_map.data.sum()))
    binsize_deg = np.abs(on.wcs.wcs.cdelt[0])
    print("thick= ",EmGal.thick.val*binsize_deg, " deg")
    res = on.wcs.wcs_pix2world(np.array([0.,EmGal.ypos.val],ndmin=2),1)
    lpix,bpix = res[0]
    print("bpos= ",str(bpix), " deg")
...
map_data.data = get_data().y.reshape(shape) * mask
model_map.data = get_model()(get_data().x0, get_data().x1).reshape(shape) * mask
print("En dehors de la zone d exclusion")
print(map_data.data.sum())
print(model_map.data.sum())
...
print("Sur toute la zone d exclusion")
print(map_data.data.sum())
print(model_map.data.sum())
"""
