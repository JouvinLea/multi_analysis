...
param_fit = input_param["param_fit"]
...
for i_E, E in enumerate(energy_bins[0:-1]):
    ...
    # load CS model
    CS = make_CS_model(outdir_data, on, param_fit["CS"]["ampl_init"], param_fit["CS"]["ampl_frozen"],
    ...
    Large_Scale = source_NormGauss2D(param_fit["Large scale"]["name"], param_fit["Large scale"]["fwhm_init"],
                              param_fit["Large scale"]["fwhm_frozen"], param_fit["Large scale"]["ampl_init"],
    ...
    name_interest = region_interest(source_center_SgrA, on, lat, lon)
    ...
    width = 100
    height = 80
    name_region = "box(" + str(source_J1745_303_xpix + 20) + "," + str(source_J1745_303_ypix - 20) + "," + str(
        width) + "," + str(height) + ")"
    ...
    # Si on inverse LS et CS alors c est qu il y a les deux!
    if param_fit["invert_CS_LS"]:
        list_src.append(psf_SgrA(exposure *gaus_CS * CS))
        if param_fit["invert_CC_LS"]:
            list_src.append(psf_SgrA(exposure *central_gauss))    
            list_src.append(psf_SgrA(exposure *Large_Scale))
        else:    
            list_src.append(psf_SgrA(exposure *Large_Scale))
            list_src.append(psf_SgrA(exposure *central_gauss))
    else:
    ...
    if param_fit["arc source"]["fit"]:
        list_src.append(psf_SgrA(exposure * arc_source))
    if param_fit["SgrB2"]["fit"]:
        list_src.append(psf_SgrA(exposure * sgrB2))
    ...
    for i_src, src in enumerate(list_src):
        ...
        save_resid(outdir_result + "/residual_step_flux_" + str(i_src) + "_"+ name + "_" + str("%.2f" % E1) + "_" + str("%.2f" % E2) + "_TeV.fits", clobber=True)

        # import IPython; IPython.embed()

    table_models = table_models.filled(-1000)
    table_covar = table_covar.filled(0)
    filename_table_result = outdir_result + "/flux_fit_result_" + name + "_" + str("%.2f" % E1) + "_" + str(
        "%.2f" % E2) + "_TeV.txt"
    table_models.write(filename_table_result, format="ascii")
    filename_covar_result = outdir_result + "/flux_covar_result_" + name + "_" + str("%.2f" % E1) + "_" + str(
        "%.2f" % E2) + "_TeV.txt"
